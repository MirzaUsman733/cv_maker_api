  ...
  try {
    ...
    if (cv_info.employment_history && cv_info.employment_history.length > 0) {
      ...
      await connection.query(
        'INSERT INTO employment_history (cv_id, job_title, employer, city, description, start_date, end_date, is_current) VALUES ?',
        [employmentData]
      );
    }
    ...
    if (cv_info.internships && cv_info.internships.length > 0) {
      ...
      await connection.query(
        'INSERT INTO internships (cv_id, job_title, company, city, description, start_date, end_date) VALUES ?',
        [internshipsData]
      );
    }
    ...
    res.status(201).json({ message: 'CV and related data created successfully' });
  } catch (error) {
    console.error(error);
...
exports.getAllCVs = async (req, res) => {
  try {
    ...
    const [employmentHistory] = await connection.query(`
      SELECT cv_id,employment_id, job_title, employer, city, description, start_date, end_date, is_current
    ...
    const [internships] = await connection.query(`
      SELECT cv_id, internship_id, job_title, company, city, description, start_date, end_date
      FROM internships
      WHERE cv_id IN (?)
    `, [cvIds]);
...
exports.getCVById = async (req, res) => {
  ...
  try {
    ...
    const [employmentHistory] = await connection.query(`
      SELECT job_title, employer, city, description, start_date, end_date, is_current
      FROM employment_history
      WHERE cv_id = ?
    `, [cv_id]);
    ...
    // Fetch internships
    const [internships] = await connection.query(`
      SELECT job_title, company, city, description, start_date, end_date
      FROM internships
      WHERE cv_id = ?
    `, [cv_id]);
...
exports.updateCV = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      await connection.query(
        'UPDATE user_data SET ? WHERE cv_id = ?',
        [{
          first_name: user.first_name,
          last_name: user.last_name,
          job_title: user.job_title,
    ...
    if (cv_info?.employment_history) {
      await connection.query('DELETE FROM employment_history WHERE cv_id = ?', [cv_id]);
      const employmentData = cv_info.employment_history.map(job => [
        cv_id,
        job.job_title,
        job.employer,
    ...
    if (cv_info?.internships) {
      await connection.query('DELETE FROM internships WHERE cv_id = ?', [cv_id]);
      const internshipsData = cv_info.internships.map(internship => [
        cv_id,
        internship.job_title,
        internship.company,
...
//     const [employmentHistory] = await connection.query(`
//       SELECT cv_id, job_title, employer, city, description, start_date, end_date, is_current
//       FROM employment_history
//       WHERE cv_id IN (?)
//     `, [cvIds]);
...
//     const [internships] = await connection.query(`
//       SELECT cv_id, job_title, company, city, description, start_date, end_date
//       FROM internships
//       WHERE cv_id IN (?)
//     `, [cvIds]);
...
exports.getAllPublicCVs = async (req, res) => {
  try {
    ...
    const limit = parseInt(req.query.limit) || 10;
    const offset = (page - 1) * limit;
    const searchJobTitle = req.query.job_title; // Job title to search for
    ...
    // Add search condition if job_title is provided
    const queryParams = [];
    if (searchJobTitle) {
      baseQuery += ` AND user_data.job_title LIKE ?`;
